...
int al_add(ArrayList* pList,void* pElement)
{
    ...
    if(pList!=NULL&&pElement!=NULL)
    {
        ...
        {
            returnAux=0;
            pList->pElements[al_len(pList)]=pElement;
            pList->size++;
        }
    }
    return returnAux;
...
int al_remove(ArrayList* pList,int index)
{
    ...
    if(pList!=NULL && index>=0 && index<pList->size )
    {
        if(!contract(pList,index)&&!resizeDown(pList))
        returnAux =0;
    }
    return returnAux;
}
...
int al_clear(ArrayList* pList)
{
    ...
    {
        returnAux=0;
        void *pAux=realloc(pList->pElements,sizeof(void*)*AL_INITIAL_VALUE);
        if(pAux!=NULL)
        {
            pList->size=0;
            free(pList->pElements);
...
/*
void* al_pop(ArrayList* pList,int index)//YO
{
    void* returnAux = NULL;
    if(pList!=NULL&&index>=0&&index<pList->size)
    {
...
int al_sortByDoubleParameter(ArrayList* pList, int (*pFunc)(void* ,void*),int (*pFunc2)(void*,void*), int order, int order2)
{
    int returnAux = -1;
    void *aux;
    if(pList!=NULL && ( order==1 || order==0 ) && pFunc!=NULL)
    {
        ...
        for(i=0;i<pList->size-1;i++)
        {
            for(j=i+1;j<pList->size;j++)
            {
                ...
                else if (pFunc(pList->pElements[i], pList->pElements[j]==0))
                {
                    if(order2==1 && pFunc2(pList->pElements[i],pList->pElements[j])>0)
                    {
                        aux=pList->pElements[i];
                        pList->pElements[i]=pList->pElements[j];
                        pList->pElements[j]=aux;
                    ...
                    else if( !order2 && pFunc2(pList->pElements[i],pList->pElements[j])<0 )
                    {
                        aux=pList->pElements[i];
                        pList->pElements[i]=pList->pElements[j];
                        pList->pElements[j]=aux;
